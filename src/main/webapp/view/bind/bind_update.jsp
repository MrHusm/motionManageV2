        ...
        <div class="row-fluid">
            ...
            <div class="span10">
              <div class="hero-unit">
                ...
                <form id ="form" action="<%=basePath%>/bind/updateBind" method="post">
                	<input type="hidden" id="id" name="id" value="${bind.id }">
                	<input type="hidden" id="money" name="money" value="${bind.money }">
	                <table class="table table-bordered table-hover">
	                	<tr>
	                		<th style="width:15%">名称：</th>
	                		<td style="width:35%"><input type="text" id="name" name="name" value="${bind.name }"></td>
	                		<th style="width:15%">绑定类型：</th>
	                		<td style="width:35%">
			              	  <select name="bindType">
			              	   	   <option value="1" ${bind.bindType == '1' ? "selected" : "" }>手机</option>	
			              	   	   <option value="2" ${bind.bindType == '2' ? "selected" : "" }>QQ</option>	
			              	   	   <option value="3" ${bind.bindType == '3' ? "selected" : "" }>微博</option>
			              	   	   <option value="3" ${bind.bindType == '4' ? "selected" : "" }>微信</option>
			              	   </select>
							</td>
	                	</tr>
	                	<tr>
	                		...
	                		<td style="width:35%">
			              	  <select name="repayType" id="repayType" onchange="showType(this)">
			              	   	   <option value="1" ${bind.repayType == '1' ? "selected" : "" }>铜币</option>	
			              	   	   <option value="2" ${bind.repayType == '2' ? "selected" : "" }>代金券</option>	
			              	   </select>
							</td>
	                		...
	                		<td name="showTypeTwo" style="display:none"><input type="text" id="cashCouponId" disabled="disabled" value="${bind.cashCouponId }" name="cashCouponId"></td>
	                	</tr>
	                	<tr name="showTypeTwo" style="display:none">
							<th>是否使用代金券金额：</th>
	                		<td>
	                		   <select name="isCashMoney" id="isCashMoney" disabled="disabled" onchange="showMoney(this)">
			              	   	   <option value="1" ${bind.isCashMoney == '1' ? "selected" : "" }>使用</option>	
			              	   	   <option value="2" ${bind.isCashMoney == '2' ? "selected" : "" }>不使用</option>	
			              	   </select>
	                		</td>
	                	...
	                	<tr>
	                		...
	                		<td>
	                		   <select name="byType" id="byType" onchange="showBy(this)">
			              	   	   <option value="1" ${bind.byType == '1' ? "selected" : "" }>不赠送</option>	
			              	   	   <option value="2" ${bind.byType == '2' ? "selected" : "" }>赠送</option>	
			              	   </select>
	                		</td>
	                		<th></th>
	                		<td></td>
	                	...
	                	<tr name="showBy" style="display:none">
	                		<th>包月ID：</th>
	                		<td>
	                		   <input type="text" disabled="disabled" id="byId" name="byId" value="${bind.byId }"/>
	                		</td>
	                		<th>赠送时间：</th>
	                		<td><input type="text" disabled="disabled" id="day" name="day" value="${bind.day }"/>&nbsp;天</td>
	                	</tr>	                	
	                	
	                	<tr>
	                		<th>开始时间：</th>
	                		<td>
								<input type="text" class="Wdate input" onFocus="WdatePicker({lang:'en',startDate:'%y-%M-01',dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'endDate\')}',alwaysUseStartDate:true})" name="startDate" id="startDate" value="${fn:substringBefore(bind.startDate, ' ')}">
							</td>
							<th>结束时间：</th>
	                		<td>
		                		<input type="text" class="Wdate input" onFocus="WdatePicker({lang:'en',startDate:'%y-%M-01',dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'startDate\')}',alwaysUseStartDate:true})" name="endDate" id="endDate" value="${fn:substringBefore(bind.endDate, ' ')}">
	                		</td>
	                	</tr>
	                	...
	                	<tr>
	                		<th>顶部文案：</th>
	                		<td colspan="3">
	                		   <input type="text" id="content" name="content" style="width:82%" value="${bind.content }"/>
	                		</td>
	                	</tr>
	                	...
	                	<tr>
	                		...
	                		<td>
	                		   <select name="channelType" id="channelType" onchange="showChannel(this)">
			              	   	   <option value="0" ${bind.channelType == '0' ? "selected" : "" }>无</option>
			              	   	   <option value="1" ${bind.channelType == '1' ? "selected" : "" }>仅在</option>	
			              	   </select>
	                		</td>
	                		<td name="showChannelTwo"></th>
	                		<td name="showChannelTwo"></td>
	                		<th name="showChannelOne" style="display:none">渠道ID：</th>
	                		<td name="showChannelOne" style="display:none"><input type="text" id="channelIds" disabled="disabled" name="channelIds" value="${bind.channelIds }"></td>
	                	</tr>
	                	<tr>
	                		<th>版本选择：</th>
	                		<td colspan="3">
	                			...
	                			<c:forEach var="ver" items="${versions }">
									<input type="checkbox" name="version" value="${ver.vcode }" ${fn:contains(bind.version, ver.vcode) ? "checked" : "" }/>${ver.vcode }&nbsp;
	                			</c:forEach>
	                		</td>
	                	</tr>
	                </table>

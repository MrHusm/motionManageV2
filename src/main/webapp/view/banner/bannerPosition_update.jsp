        ...
        <div class="row-fluid">
            ...
            <div class="span10">
              <div class="hero-unit">
                ...
                <form id ="form" action="<%=basePath%>/jpBannerPosition/updateBannerPosition" method="post">
                	<input type="hidden" id="id" name="id" value="${position.id }">
                	<input type="hidden" name="dqPage" value="${dqPage }" />
                	<input type="hidden" name="source" value="${position.source }" />
	                <table class="table table-bordered table-hover">
	                	<tr>
	                		<th style="width:15%">广告位名称：</th>
	                		<td style="width:35%"><input type="text" id="name" name="name" value="${position.name }"></td>
	                		<th style="width:15%">广告类型：</th>
	                		<td style="width:35%">
			              	  <select name="type" id="type">
			              	   	   <option value="1" ${position.type == '1' ? "selected" : "" }>大图</option>	
			              	   	   <option value="2" ${position.type == '2' ? "selected" : "" }>小图</option>
			              	   	   <option value="3" ${position.type == '3' ? "selected" : "" }>文字</option>
			              	   	   <option value="4" ${position.type == '4' ? "selected" : "" }>推广</option>
			              	   </select>
							</td>
	                	</tr>
	                </table>
	                ...
	                <div align="center">
	                   ...
	                   <input type="button" class="btn-info" name="bt" value="返回" onclick="window.location.href='<%=basePath%>/jpBannerPosition/listBannerPosition?dqPage=${dqPage}&source=${position.source }'">
	                </div>
                </form> 
            </div><!--/span-->
          </div><!--/row-->
